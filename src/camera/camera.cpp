...
void mouse_callback(GLFWwindow* window, double xpos, double ypos) {
    if (!rightMousePressed) return; // Solo rotar si el botón derecho está presionado
    ...
    targetYaw += xoffset;
    targetPitch += yoffset;
    if (targetPitch > 80.0f) targetPitch = 80.0f;
    if (targetPitch < -80.0f) targetPitch = -80.0f;
...
void update_camera_direction() {
    ...
    if (camYaw > 720.0f) camYaw -= 360.0f;
    if (camYaw < -720.0f) camYaw += 360.0f;
    float yawR = camYaw * 3.1415926f / 180.0f;
    float pitchR = camPitch * 3.1415926f / 180.0f;
    float dir[3] = { cosf(yawR) * cosf(pitchR), sinf(pitchR), sinf(yawR) * cosf(pitchR) };
...
void update_camera_position(float dt, GLFWwindow* window) {
    float speed = (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS) ? 6.0f : 3.0f;
